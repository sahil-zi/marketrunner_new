...
function generateQRLayout(barcode, style, size, orderNumber) {
  ...
  // Barcode text and order number left-aligned from x=10 (QR left edge), full available width
  const orderField = orderNumber ? `^FO10,241^A0N,24,24^FB380,1,0,L^FD${orderNumber}^FS` : '';
  return `^XA^PW400^LL${ll}^CI28
^FO10,40^BQN,2,7^FDQA,${barcode}^FS
^FO10,215^A0N,24,24^FB380,1,0,L^FD${barcode}^FS
${orderField}
...
function generateBarcodeLayout(barcode, style, size, orderNumber) {
  ...
  const orderField = orderNumber ? `^FO60,155^A0N,24,24^FB320,1,0,L^FD${orderNumber}^FS` : '';
...
^FO60,130^A0N,24,24^FB320,1,0,L^FD${barcode}^FS
${orderField}
