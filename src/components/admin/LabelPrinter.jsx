...
function generateQRLayout(barcode, style, size, orderNumber) {
  ...
  // Barcode text and order number are centered within the QR code's width (~175 dots from x=10)
  const orderField = orderNumber ? `^FO10,241^A0N,24,24^FB175,1,0,C,0^FD${orderNumber}^FS` : '';
  return `^XA^PW400^LL${ll}^CI28
^FO10,40^BQN,2,7^FDQA,${barcode}^FS
^FO10,215^A0N,24,24^FB175,1,0,C,0^FD${barcode}^FS
${orderField}
...
function generateBarcodeLayout(barcode, style, size, orderNumber) {
  ...
  const orderField = orderNumber ? `^FO60,155^A0N,24,24^FB240,1,0,C,0^FD${orderNumber}^FS` : '';
...
^FO60,130^A0N,24,24^FB240,1,0,C,0^FD${barcode}^FS
${orderField}
^FO100,${styleY}^A0N,25,25^FB150,1,0,L^FDStyle:^FS
^FO100,${nameY}^A0N,25,25^FB150,2,0,L^FD${style}^FS
^FO100,${sizeY}^A0N,25,25^FB150,1,0,L^FDSize: ${size}^FS
^XZ`;
}
